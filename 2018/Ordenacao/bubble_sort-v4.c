...
void bubble_sort_int(int tamanho, int vetor[])
{
    //considerando que a ultima posicao do vetor esta ordenada
    int i, j, ultimo, houvetroca;
    ultimo = tamanho - 1;
...
void exibe_vetor(int tamanho, int v[])
{ //Exibe
    int t;
    for (t = 0; t < tamanho; t++)
        printf("%-4d ", v[t]);
...
int aleatorio(int n)
{ //funcao para gerar aleatorios inteiros
    return rand() % n;
}
