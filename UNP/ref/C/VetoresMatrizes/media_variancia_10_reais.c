...
void preenche_vetor(float z[], int tamanho);
...
float variancia(int n, float *v, float med)
{            // calcula a variancia dada a media
...
void exibe_vetor(float v[])
{ // Exibe o vetor do tipo float
    int t;
    printf("\nO vetor digitado eh\n\n");
    for (t = 0; t < MAX; t++)
        printf("%-3g ", v[t]);
}
void preenche_vetor(float vet[], int tamanho)
{ // Preenche o vetor
    int i;
    for (i = 0; i < tamanho; ++i)
