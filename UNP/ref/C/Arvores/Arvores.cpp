/*  Função : Exemplo do uso de Arvores Binarias
    Autor : Edkallenn
    Data : 10/06/2015
    Observações: Implementacoes recursivas. Info é um char.
*/
#include <stdio.h>
#include <stdlib.h>
...
main()
{
    ...
    */
    exibicoesArvores(a); // exibicoes em pr�-ordem
    printf("\nAcrescentando itens de forma recursiva!\n");
    printf("Pressione Enter quando estiver pronto");
    getchar();
    // pode-se acrescentar n�s da seguinte forma (usando a definicao recursiva de criaArvore
    // assim:
    ...
    a->dir->esq = liberaArvore(a->dir->esq);
    pula(2);
    exibicoesArvores(a);

    char elemento;
    int opcao;
    printf("Qual elemento vc deseja buscar? ");
    scanf(" %c", &elemento);
...
void imprimeArv(Arv *a)
{ // Obs.: imprime em pr�-ordem, como a definicao de arvore:
    if (!arvoreVazia(a))
    {                           //      primeiro: raiz, depois esqueda em seguida direita.
        printf("%c ", a->info); // raiz
        imprimeArv(a->esq);     // sub�rvore esquerda
        imprimeArv(a->dir);     // subarvore direita
    }
}
void imprimeArvNotacao(Arv *a)
{ // tamb�m imprime em pr�-ordem.
    printf("<");
    if (!arvoreVazia(a))
    {
        printf("%c", a->info);     // raiz
        imprimeArvNotacao(a->esq); // sub�rvore esquerda
...
Arv *liberaArvore(Arv *a)
{
    if (!arvoreVazia(a))
    {
        liberaArvore(a->esq); // sub�rvore esquerda
        liberaArvore(a->dir); // subarvore direita
        free(a);
    }
    return NULL;
    // observa��o ---> Usa p�s-ordem! Primeiro subArvore esq, depois SubArvore direita,
    //                 em seguida, a raiz.
}
int buscaArvore(Arv *a, char c)
{
    if (arvoreVazia(a))
        ...
        return a->info == c ||
               buscaArvore(a->esq, c) ||
               buscaArvore(a->dir, c);
    /* comentario sobre a funcao busca: ela interrompe a busca t�o logo o elemento
    seja encontrado. Se a primeira express�o for verdadeira as duas outras n�o chegam
    a ser avaliadas.
    */
}

void imprimeArvEmOrdem(Arv *a)
{ // Obs.: imprime em ordem:
    if (!arvoreVazia(a))
    {                           //      primeiro: subArv Esq, depois raiz em seguida SubArvDireita.
        imprimeArv(a->esq);     // sub�rvore esquerda
        printf("%c ", a->info); // raiz
        imprimeArv(a->dir);     // subarvore direita
    }
...
void imprimeArvPosOrdem(Arv *a)
{ // Obs.: imprime em pos-ordem:
    if (!arvoreVazia(a))
    {                           //      primeiro: subArv Esq, depois SubArvDireita em seguida raiz.
        imprimeArv(a->esq);     // sub�rvore esquerda
        imprimeArv(a->dir);     // subarvore direita
        printf("%c ", a->info); // raiz
    }
...
// A fun��o mostraArvore faz um desenho esquerda-direita-raiz
// da �rvore x. O desenho ter� uma margem esquerda de
// 3b espa�os.
void mostraArvore(Arv *a, int b)
{
    if (a == NULL)
...
// A fun��o auxiliar imprimeNo imprime o caracter
// c precedido de 3b espa�os e seguido de uma mudan�a
// de linha.
void imprimeNo(char c, int b)
{
    int i;
...
void exibicoesArvores(Arv *a)
{
    ...
    printf("\nA altura desta arvore = %d", alturaArvore(a));
    pula(1);
