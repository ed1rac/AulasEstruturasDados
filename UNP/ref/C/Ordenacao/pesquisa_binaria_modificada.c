...
/*  Função : Pesquisa usando busca binaria (vetores)
    ...
    Obs: Esta é a versãoo mais simples, mas com funcoes */
...
void preenche_vetor_aleatorio(int tamanho, int num, int vet[]);
...
main(){
    ...
    preenche_vetor_aleatorio(TAM, 1000, vetor); //preenche vetor - mesma funcao anterior
    printf("\n\nO vetor digitado eh\n");
...
void preenche_vetor_aleatorio(int tamanho, int num, int vet[]){
    int i, valor; // Preenche com valores aleatorioicos melhor
    for (i=0;i<tamanho;++i){
        valor = (1 + aleatorio(num-1)); 	//gera ate num
        vet[i]=valor;
    }
}
