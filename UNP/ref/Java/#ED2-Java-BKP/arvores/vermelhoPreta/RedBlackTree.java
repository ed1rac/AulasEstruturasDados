 ...
 * @author Ignas Lelys
 * @created May 6, 2011
 * @implNote Esta implementa��o � baseada no livro Cormen. Criada por Ignas Lelys.
 *           Alterada por Ed em 11/10/2023. Foram mantidos os nomes de m�todos originais
 *           Foi alterado o m�todo printNodeValue tanto na classe m�e quanto nesta classe
 *           para indicar a COR do n�. Isso facilita a visualiza��o da �rvore
 */
public class RedBlackTree extends AbstractSelfBalancingBinarySearchTree {
    ...
    protected void printNodeValue(Node node) {
        ...
        final String BG_BRANCO = "\u001B[47m";

        if (node.value == null) {
            System.out.print("<null>");
        } else {
            if (((RedBlackNode)node).color == ColorEnum.BLACK) {
                System.out.print(BG_BRANCO + PRETO + node.value.toString() + "(B)" + RESET);
            }else{
                System.out.print(VERMELHO + node.value.toString() + "(R)" + RESET);
            }
        }
        System.out.println();
    }
